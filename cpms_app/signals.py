...
from .models import Profile, Team, Notification, Milestone, User, Project, ChatRoom,TaskStep,Task
...
@receiver(m2m_changed, sender=Milestone.assigned_to.through)
def notify_milestone_assignment(sender, instance, action, pk_set, **kwargs):
    if action == "post_add":
        ...
        with transaction.atomic():
            for user_id in pk_set:
                ...
                Notification.objects.create(
                    recipient=user,
                    message=f"You have been assigned a new milestone: {instance.name}."
                )

@receiver(pre_delete, sender=Milestone)
def notify_users_milestone_deleted(sender, instance, **kwargs):
    # Get the users assigned to the milestone before deletion
    assigned_users = instance.assigned_to.all()

    if not assigned_users:
        print(f"No users were assigned to milestone '{instance.name}'.")
        return
    ...
    for user in assigned_users:
        Notification.objects.create(
            recipient=user,
            message=f"The milestone '{instance.name}' you were assigned to has been deleted."
        )
        print(f"Notification sent to {user.username}: 'The milestone {instance.name} was deleted.'")

@receiver(pre_save, sender=Project)
def revoke_milestones_on_team_change(sender, instance, **kwargs):
    if instance.pk:  # Ensures we're updating an existing project
        ...
        if previous_project.assigned_team and previous_project.assigned_team != instance.assigned_team:
            # Get milestones assigned to members of the previous team
            milestones_to_revoke = Milestone.objects.filter(
                project=instance,
                assigned_to__in=previous_project.assigned_team.members.all()
            )

            # Revoke milestones from the previous team's members
            for milestone in milestones_to_revoke:
                milestone.assigned_to.clear()  # Removes all assigned users from the milestone
                milestone.is_active = False    # Optionally, deactivate the milestone
                milestone.save()
...
def update_project_status(sender, instance, **kwargs):
    """Update the status of a project when a milestone changes."""
...
def update_milestone_status(sender, instance, **kwargs):
    """Update the status of a milestone when a task changes."""
    task = instance.milestone
    task.save()  # This will call the custom save method in the milestone model
@receiver(post_save, sender=TaskStep)
def update_task_status(sender, instance, **kwargs):
    """Update the status of a task when a step changes."""
    step = instance.task
    step.save()  # This will call the custom save method in the task model
