...
def other_landing_page(request):
    ...
    recent_tasks = Task.objects.filter(Q(assigned_to=user) , Q(updates__updated_by=user), Q(status__in=["pending", "in_progress"] )).select_related("project").distinct().order_by("-updates__created_at")[:5]

    # Task section
    tasks=Task.objects.filter(Q(assigned_to=user))
    total_tasks=tasks.count()
    ongoing_tasks = tasks.filter(assigned_to=user, status="in_progress")
    pending_tasks = tasks.filter(assigned_to=user, status="pending")
    completed_tasks = tasks.filter(assigned_to=user, status="completed")
...
def admin_activity_view(request):
    ...
    # Apply task filter
    if task_filter:
        activities = activities.filter(object_id=task_filter, activity_type__in=['task_created', 'task_updated', 'task_deleted'])
...
def project_timeline():
    data = {
        "timestamp": [
            "2025-01-20 08:30", "2025-01-20 08:40",
            "2025-01-20 09:00", "2025-01-20 09:10",
            "2025-01-20 09:20", "2025-01-20 09:30"
        ],
        "activity": ["Login", "Upload", "Download", "Login", "Upload", "Logout"],
    ...
    # Add vertical line for the timeline
    fig.add_trace(go.Scatter(
        x=[0] * len(df),  # All points at x=0 for the vertical timeline
        y=df['timestamp'],
        mode="lines",
        line=dict(color="black", width=2),  # Black vertical line
        showlegend=False
    ))

    # Add branches with colored circles and annotations
    for i, row in df.iterrows():
        if i % 2 == 0:
            ...
            fig.add_trace(go.Scatter(
                ...
                marker=dict(size=10, color="blue", symbol="circle"),  # Circle marker
                showlegend=False
            ))
            # Add annotation above the left circle
            fig.add_annotation(
                x=-1,  # Centered on the circle
                y=row['timestamp'],
                text=f"{row['activity']} ({row['user']})<br>{row['details']}",
                showarrow=False,
                font=dict(size=10),
                align="center",
            ...
            # Right branch
            fig.add_trace(go.Scatter(
                x=[0, 1],  # From center to the right
                y=[row['timestamp']] * 2,
                mode="lines+markers",  # Add markers
    ...
    fig.update_layout(
        ...
        margin=dict(l=200, r=200)  # Add padding for branches
    )
