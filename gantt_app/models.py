...
from django.contrib.auth.models import AbstractUser, Group, Permission  # For assigning tasks to users or teams
...
class Project(models.Model):
    ...
    def calculate_progress(self):
        all_tasks = self.tasks.filter(start_date__isnull=False, end_date__isnull=False)
        if not all_tasks.exists():
            return 0  # No tasks mean 0% progress

        total_duration = sum(max(1, (task.end_date - task.start_date).days + 1) for task in all_tasks)
        completed_duration = sum(max(1, (task.end_date - task.start_date).days + 1)
                                 for task in all_tasks if task.status == 'completed')

        return round((completed_duration / total_duration) * 100, 2) if total_duration > 0 else 0
    ...
    def update_status_based_on_tasks(self):
        """Update project status based on the task completion."""
        ...
        if completed_tasks.count() == total_tasks and total_tasks != 0:
            ...
            self.status = 'pending'
    def save(self, *args, **kwargs):
        self.update_status_based_on_tasks()

        # Save the instance with updated status
        super().save(*args, **kwargs)
...
class Task(models.Model):
    ...
    end_date = models.DateField()
    can_be_extended = models.BooleanField(default=False)
    assigned_to = models.ManyToManyField(User, related_name="tasks", blank=True)  # Multiple users can be assigned
    project = models.ForeignKey('Project', on_delete=models.CASCADE, related_name="tasks")
    status = models.CharField(max_length=20, choices=STATUS_CHOICES, default="pending")
    assigned_group = models.ForeignKey('Team', on_delete=models.SET_NULL, null=True, blank=True, related_name="tasks")
    description = models.TextField(null=True, blank=True)
    ...
    def assign_task_to_group(self):
        """ Assign the task to all members of the group """
        if self.assigned_group:
            self.assigned_to.set(self.assigned_group.members.all())  # Assign task to all members of the team
            self.save()
    @property
...
class Milestone(models.Model):
    ...
    def update_milestone_status(self):
        """ Check if all tasks in the project linked to this milestone are completed """
        tasks = self.project.tasks.filter(milestone=self)
        if tasks.filter(status="completed").count() == tasks.count():
            self.is_completed = True
            self.actual_completion_date = timezone.localtime().date()
            self.save()
