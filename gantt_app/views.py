...
def other_landing_page(request):
    ...
    completed_projects = projects.filter(Q(status='completed')).count()
    pending_projectss = projects.filter(Q(status='pending')).count()
    ongoing_pending_projects = projects.filter(Q(status__in=["pending", "in_progress"])).order_by("-updated_at")
    user_projects = pending_projects = projects.filter(Q(status='in_progress')).count()
    ...
    ongoing_tasks = tasks.filter(assigned_to=user, status="in_progress").count()
    pending_tasks = tasks.filter(assigned_to=user, status="pending").count()
    completed_tasks = tasks.filter(assigned_to=user, status="completed").count()
    ...
    context = {
        "teams":teams,
        "ongoing_pending_projects": ongoing_pending_projects,  
        "projects":projects, 
        ...
        "pending_projectss":pending_projectss,
    }
...
def team_list(request):
    ...
    search_query = request.GET.get('search', '')
    projects = Project.objects.filter(status__in=["pending","in_progress"])
